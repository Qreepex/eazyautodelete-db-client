...
import { ChannelSettings, DatabaseHandlerConfig, FilterType, FilterUsage, GuildSettings, ModeType, UserSettings, UserSettingsLanguage } from "../typings";
...
export default class DatabaseHandler {
    ...
    async createUserSettings(userId: string, lang: UserSettingsLanguage, registered: number) {
    ...
    async updateUserSettings(userId: string, lang: UserSettingsLanguage, registered: number) {
        let data = await this.mongo.getUserSettings(userId);
        let formattedData: UserSettings = {
            id: data.id,
            registered: data.registered,
    ...
    async createGuildSettings(guildId: string, registered: number = new Date().getTime(), prefix: string = "%", premium: boolean = false, adminroles: Array<string> = [], modroles: Array<string> = []) {
    ...
    async updateGuildSettings(guildId: string, registered: number = new Date().getTime(), prefix: string = "%", premium: boolean = false, adminroles: Array<string> = [], modroles: Array<string> = []) {
        let data = await this.mongo.getGuildSettings(guildId);
        let formattedData = {
            id: data.id,
            registered: data.registered,
    ...
    async createChannelSettings(channelId: string, guild: string, registered: number = new Date().getTime(), limit: number = 0, mode: ModeType = 0, ignore: Array<string> = [], filters: Array<FilterType> = [], regex: RegExp | null = null, filterUsage: FilterUsage = "one"): Promise<ChannelSettings> {
    ...
    async updateChannelSettings(channelId: string, guild: string, registered: number = new Date().getTime(), limit: number = 0, mode: ModeType = 0, ignore: Array<string> = [], filters: Array<FilterType> = [], regex: RegExp | null = null, filterUsage: FilterUsage = "one") {
        let data = await this.mongo.getChannelSettings(channelId);
        let formattedData = {
            id: data.id,
            guild: data.guild,
